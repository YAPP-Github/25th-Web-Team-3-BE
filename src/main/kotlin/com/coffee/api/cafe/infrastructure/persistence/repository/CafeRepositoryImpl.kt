...
class CafeRepositoryImpl(
    ...
    override fun findAllCafesById(lastCafeId: UUID?, limit: Int) : Slice<Cafe> {
        ...
        val result = listQuery.take(limit).map { converter.toDomain(it) }
        return SliceImpl(result, Pageable.unpaged(), hasNext)
    }
}